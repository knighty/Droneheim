...
namespace Droneheim
{
	...
	public class FloatKeyframes : InterpolatedKeyframeList<float>, Interpolator<float>
	{
	...
	public class ValheimMod : BaseUnityPlugin
	{
		...
		[HarmonyPrefix]
		static void qwdqwd(ref VertexHelper toFill, ref Image __instance, ref bool __runOriginal)
		{
			if (__instance.sprite != tempSprite) { return;  }
			Debug.Log(__instance.sprite != null ? __instance.sprite.name : "Missing");
			Debug.Log(__instance.type);
			__runOriginal = true;
		}

		[HarmonyPatch(typeof(Image), "OnPopulateMesh")]
		[HarmonyPostfix]
		static void qwdsdasdqwd(ref VertexHelper toFill, ref Image __instance)
		{
			if (__instance.sprite != tempSprite) { return; }
			Debug.Log(toFill.currentVertCount);
		}*/
	}
